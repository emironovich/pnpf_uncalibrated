path = 'data/eth3d_results_40_140_26_06/loransac/delivery_area';
...
%make_bench_before_ba('p35p', path, path_gt, output_file_name);
make_bench_before_ba('p4p', path, path_gt, output_file_name);


function make_bench_before_ba(solver_name, input_path, input_path_gt, output_file_name)
    [cameras_eval, images_eval, ~] = read_model([input_path '/' solver_name '/2']);
    [cameras_gt, images_gt, ~] = read_model(input_path_gt);
    
    [cameras_before_ba, images_before_ba] = read_before_ba([input_path '/' solver_name '/2'], cameras_eval, images_eval);
    
    [f_diff, R_diff, t_diff] = cmp_solver_absolute_transformation(cameras_gt, images_gt, cameras_before_ba, images_before_ba);
