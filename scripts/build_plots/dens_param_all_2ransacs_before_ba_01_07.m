...
for ind_bench = 1 : numel(bench_names)
    ...
    for ind_method = 1 : numel(methods)      
        ...
        combined_name = ['combined_before_ba_' base_name];
        ...
        a = min([min(log10(p35p(:,param_num))) min(log10(p4p(:,param_num))) min(log10(real(:,param_num)))]) - 0.5;
        b = max([max(log10(p35p(:,param_num))) max(log10(p4p(:,param_num))) max(log10(real(:,param_num)))]) + 0.5;
        x_values = a:0.1:b;
   ...
   linkaxes([ax{1},ax{2},ax{4},ax{5}],'y');
   if param_name == 'f'
        title(t, ['log_{10} относительной ошибки фокусного расстояния, ВЗЯТОГО НЕПОСРЕДСТВЕННО ПОСЛЕ РАБОТЫ RANSAC, на датасете ' bench_name ' из eth3d']);
   elseif param_name == 'R'
        ...
        saveas(fig, ['combined_before_ba_' bench_name '_' res_date '_' param_name '.png']);
        print(fig, ['combined_before_ba_' bench_name '_' res_date '_' param_name], '-depsc');
end
close all;