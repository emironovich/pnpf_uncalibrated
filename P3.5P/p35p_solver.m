function [solution_num, f_sol, R_sol, T_sol] = p35p_solver(X, x, y, e)
    type = class(X);
    assert(strcmp(type, class(y)));
    assert(strcmp(type, class(x)));
    assert(strcmp(type, class(e)));
    solution_num = int32(0);
    f_sol = zeros(1, 0, 'like', X);
    coder.varsize('f_sol', [1 10], [0 1]);
    R_sol = zeros(3, 3, 0, 'like', X);
    coder.varsize('R_sol', [3 3 10], [0 0 1]);
    T_sol = zeros(3, 0, 'like', X);
